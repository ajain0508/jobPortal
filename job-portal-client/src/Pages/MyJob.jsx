import React, { useEffect, useState } from 'react'
import { set } from 'react-hook-form'
import { Link } from 'react-router-dom'

const MyJob = () => {
    const email = "adarshjain0508@gmail.com"
    const [jobs,setJobs] = useState([])
    const [isLoading,setIsLoading] = useState(true)
    const [searchtext,setSearchText] = useState("")
    const [currentPage,setCurrentPage] = useState(1)
 
   
    // eslint-disable-next-line react-hooks/exhaustive-deps
   

    useEffect(()=>{
        setIsLoading(true)
        fetch(`http://localhost:5000/myJobs/${email}`)
        .then(res=>res.json())
        .then(result=>setJobs(result))
        // result is an array of jobs posted by particular email
        setIsLoading(false)
    },[])

    


    // split and join is done tom remove spaces from the string 
    // therfore if any extra space is entered then also we can get the result
    let filteredJobs = jobs.filter((job)=> job.jobTitle.toLowerCase().split(" ").join("").indexOf(searchtext.toLowerCase().split(" ").join("")) !== -1);


    // no need for this code we can directly use filteredJobs     
    // const filteredData = (jobs, searchtext)=>{
    //     let filtereditems = jobs;
    //     if(searchtext){
    //         filtereditems = filteredJobs
    //     }
    //     return filtereditems
    // }

    // const result = filteredData(jobs,searchtext)


    // deleting the job
    const handleDelete = (id)=>{
        
        fetch(`http://localhost:5000/job/${id}`,{
          method:'DELETE'
        })
        .then(res=>res.json())
        .then(data=>{
          if(data.acknowledged === true){
              alert("Job Deleted Successfully")
          }
          window.location.reload();
        })
             
      }   
    // pagenation
    const noOfItemsPerPage = 4
    const indexOfLastItem = currentPage*noOfItemsPerPage
    const indexofFirstItem = indexOfLastItem - noOfItemsPerPage
    const currentJobs = filteredJobs.slice(indexofFirstItem,indexOfLastItem)
    

    const prevPage = ()=>{
        if(currentPage > 1){
            setCurrentPage(currentPage-1)
        }
    }
    
    const nextPage = ()=>{
        if(currentPage<Math.ceil(filteredJobs.length/noOfItemsPerPage)){
            setCurrentPage(currentPage+1)
        }
    }

    

  return (
    <div className='max-w-screen-2xl container mx-auto px-4 xl:px-24'>
      <div className="my-jobs-container">
        <h1 className='text-center p-4'> ALL MY JOBS</h1>
        <div className='text-center p-2 mb-2'>
            <input type="text" name="search" id="search" className='py-2 pl-3 border focus:outline-none lg:w-1/2 mb-4 w-full' onChange={(e)=>setSearchText(e.target.value)} value={searchtext}/>
            <button className='bg-blue text-white font-semibold px-8 py-2 rounded-sm mb-4' >Search</button>
        </div>
      </div>

     {/* Table  */}
     <section className="py-1 bg-blueGray-50">
    <div className="w-full xl:w-8/12 mb-12 xl:mb-0 px-4 mx-auto mt-5">
    <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded ">
        <div className="rounded-t mb-0 px-4 py-3 border-0">
        <div className="flex flex-wrap items-center">
            <div className="relative w-full px-4 max-w-full flex-grow flex-1">
            <h3 className="font-semibold text-base text-blueGray-700">All Jobs</h3>
            </div>
            <div className="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
            <Link to='/post-job'><button className="bg-indigo-500 text-white active:bg-indigo-600 text-xs font-bold uppercase px-3 py-1 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button">Post a New Job</button></Link>
            </div>
        </div>
        </div>

        <div className="block w-full overflow-x-auto">
        <table className="items-center bg-transparent w-full border-collapse ">
            <thead>
            <tr>
                <th className="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                            NO.
                            </th>
            <th className="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                            Job Title
                            </th>
            <th className="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                            Company Name
                            </th>
            <th className="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                            Salary
                            </th>
            <th className="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                            Edit
                            </th>
            <th className="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                            Delete
                            </th>
            </tr>
            </thead>
            {/* currentJobs is array which coinatins jobs based on searchText and sliced according to pagination*/}
            {   
                isLoading?(<div className='flex items-center justify-center h-20'>Loading...</div>):(<tbody>
            {
                currentJobs.map((job,index)=>(<tr key={index}>
                <th className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left text-blueGray-700 ">
                {index+1}
                </th>
                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 ">
                {job.jobTitle}
                </td>
                <td className="border-t-0 px-6 align-center border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                {job.companyName}
                </td>
                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                <i className="fas fa-arrow-up text-emerald-500 mr-4"></i>
                {`\u20B9' ${job.minPrice}k`} - {`\u20B9' ${job.maxPrice}k`}
                </td>
                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                <i className="fas fa-arrow-up text-emerald-500 mr-4"></i>
                <button><Link to={`/edit-job/${job?._id}`}>Edit</Link></button>
                </td>
                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                <i className="fas fa-arrow-up text-emerald-500 mr-4"></i>
                <button className='bg-red-700 py-2 px-6 text-white rounded-sm' onClick={()=> handleDelete(job?._id)}>Delete</button>
                </td>
            </tr>))
            }
            
            
            </tbody>)
            }
            

        </table>
        </div>
    </div>
    </div>
    {/* pagination */}
    <div className='flex justify-center text-black space-x-8'>
        {
            currentPage > 1 && (
                <button className="hover:underline" onClick={prevPage}>Previous</button>
            )
         }
         {
            indexOfLastItem < filteredJobs.length && (
                <button className="hover:underline" onClick={nextPage}>Next</button>
            )
         }
    </div>
    </section>
    </div>
  )
}

export default MyJob
